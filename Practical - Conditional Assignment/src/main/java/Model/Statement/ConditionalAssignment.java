...
public class ConditionalAssignment implements IStmt{
    ...
    public MyIDictionary<String, Type> typeCheck(MyIDictionary<String, Type> typeEnv) throws MyException {
        ...
        if (type1.equals(new BoolType()) && type2.equals(variableType) && type3.equals(variableType))
            ...
            throw new MyException("This assignment is not valid!");
    }

    @Override
    public PrgState execute(PrgState state) throws MyException {
        MyIStack<IStmt> exeStack = state.getStk();
        IStmt newIf = new IfStmt(expression1, new AssignStmt(variable, expression2), new AssignStmt(variable, expression3));
        exeStack.push(newIf);
